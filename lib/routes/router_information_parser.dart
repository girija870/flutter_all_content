...
///parseRouteInformation will convert the given route information into parse data to pass to RouterDeleaate.
class RoutesInformationParser extends RouteInformationParser<RoutePath> {
  @override
  Future<RoutePath> parseRouteInformation(
      RouteInformation routeInformation) async {
    //converting the url into the custom class T (RoutePath)
    final uri = Uri.parse(routeInformation.location!);
    if (uri.pathSegments.isEmpty) {
      return RoutePath.home('/');
    }

    ///for the query - params pass the complete path
